...
const AddMoviePage = ({
  ...
  useEffect(() => {
    ...
    if (isAuthenticated && password && !state.isPasswordComplete) {
  ...
  const handleMovieIdSubmit = async (movieId) => {
    ...
    const duplicateMovie = allMovies.find(
      (movie) => movie.id === Number(movieId)
    );
    if (duplicateMovie) {
      dispatch({
        type: "MOVIE_ID_ERROR",
        error: `Movie with ID: ${movieId} already exists.`
      });
      return;
    }
    try {
  ...
  const handleSubmit = (e) => {
    ...
    handleMovieIdSubmit(movieIdInput).then(() =>
    ...
    <div className="add-movie-page">
      ...
      <form onSubmit={handleSubmit}>
        <div className="add-movie-page-inputs">
          {!isAuthenticated && (
            <div>
              <label htmlFor="password" className="header-style-font">
                Password
              </label>
              <input
                type="password"
          ...
          {!state.isIdValid && (
            <div>
              <label htmlFor="movie-id" className="header-style-font">
                Movie ID
              </label>
              <input
                type="number"
                ...
                autoComplete="number"
                onChange={(e) =>
                  dispatch({ type: "UPDATE_MOVIE_ID", value: e.target.value })
                }
...
export const mapDispatchToProps = (dispatch) =>
  bindActionCreators(
    { setBackgroundClass, setPassword, setCurrentMovie, setRemainingAttempts },
    dispatch
  );
